# DuckDB ODBC Extension × net/http Demo (Go)

> Go edition. For the same demo built with Spring Boot, see
...
```
net/http → duckdb-go (in-process, cgo) → odbc extension → unixODBC → MariaDB ODBC driver → MySQL
```

Every hop in that chain exists for a reason:

- **net/http → duckdb-go** — the `github.com/duckdb/duckdb-go/v2` module embeds
  the database as a cgo binding and exposes it through the standard
  `database/sql` interface; there is no separate JDBC driver in the Go world,
  the module *is* the client.
- **DuckDB (in-process)** — DuckDB is an embedded database: it runs inside
  the Go process as a library, so there is no DuckDB server to deploy or
  manage.
...
Everything that must be in place before `go run .` works, and why:

1. **Go 1.24+ with cgo** — the `duckdb-go/v2` module requires Go 1.24 and
   links a bundled static `libduckdb` through cgo, so builds need
   `CGO_ENABLED=1` (the default) and a C toolchain. On macOS that is the
   Xcode Command Line Tools (`xcode-select --install`); on Debian/Ubuntu it is
   `build-essential`. DuckDB itself needs no separate install: the module
   bundles the native library and links it into your binary, so the only
   runtime download is the `odbc` extension on first use. Check your version
   with `go version`; if it is older than 1.24, Go's toolchain mechanism can
   fetch the right one automatically (`GOTOOLCHAIN=go1.24.0 go run .`).
...
3. **unixODBC driver manager:**
   ...
   *Additional notes:*
   ...
   - The package also installs two diagnostic tools you'll want:
     ...
     DuckDB and Go, which cleanly separates "ODBC is broken" from
     "the app is broken".
...
4. **A MySQL-compatible ODBC driver:**
   ...
   *Additional notes:*
   ...
   - The driver's architecture must match the *process* loading it: a
     64-bit Go binary (hence 64-bit DuckDB) can only load a 64-bit driver `.so`.
   ...
   *Additional notes:*
   ...
   - `odbcinst.ini` (drivers) is not `odbc.ini` (DSNs). This demo uses a
     **DSN-less** connection string — driver name plus `Server`, `Port`,
     ...
     out of `config.go`, at the cost of another host-specific file.
...
All connection details live in `internal/demo/config.go` and can be overridden
with `APP_*` environment variables, which keeps the connection details out of
the source the same way the Java demo's `application.yml` does:

```go
DuckDBDatabase          = ":memory:"                    // APP_DUCKDB_DATABASE
ODBCConnectionString    = "Driver={MariaDB};Server=127.0.0.1;Port=3306;Database=demo"
ScannerConnectionString = "host=127.0.0.1 port=3306 user=app_user password=app_password database=demo"
...
`internal/demo/duckdb_conn.go` opens a **single shared DuckDB connection** at
startup — it opens the in-memory database through `database/sql` and pins the
pool to one physical connection (`SetMaxOpenConns(1)`) — and the ODBC connection
is opened at startup from `ODBCConnectionString` plus the credentials.
...
# 2. Fetch dependencies (the bundled libduckdb is pulled in as a module).
go mod download

# 3. Start the app (CGO_ENABLED=1 is the default; needs a C toolchain).
go run .
```
...
- **`SET VARIABLE` is per-DuckDB-connection.** The ODBC handle stored via
  ...
  that set it. This is exactly why the demo pins `database/sql` to a single
  underlying connection (`SetMaxOpenConns(1)` in `duckdb_conn.go`) instead of
  letting the pool open several: with `:memory:` each new connection is its
  *own* in-memory database, so separate connections wouldn't even share tables
  or loaded extensions. Pinning to one connection also serializes statements,
  so the session-variable state can't interleave across concurrent requests.
  For production, open the ODBC connection per statement instead (pass a
  connection string directly to `odbc_query`/`odbc_copy` with
  `close_connection = true` — stateless, therefore safe to parallelize).

- **`database/sql` scans DECIMAL as a driver struct.** The `duckdb-go` driver
  returns DuckDB `DECIMAL` as a `duckdb.Decimal{Width, Scale, *big.Int}`
  value, which would serialize to JSON as an ugly nested object. The demo's
  generic row reader converts it with `.String()` (hence `revenue` comes back
  as a string like `"125000.50"`), matching the Node and Python demos.
...
- **`odbc_query`'s SQL argument is a constant, not a bind parameter.** The
  ...
  `topCustomers` is built into the SQL string in Go (it's an `int`, so it's
...
- **The scanner ATTACH is one-time per session.** Attaching twice with the
  same alias fails, hence the lazy, guarded (`sync.Once`)
...
- **Single DuckDB connection** (`internal/demo/duckdb_conn.go`): the ODBC
  handle lives in a DuckDB session variable (`SET VARIABLE conn =
  odbc_connect(...)`), which is scoped to one connection — see Gotchas. The
  demo uses `database/sql` but pins the pool to one physical connection with
  `SetMaxOpenConns(1)`, which both preserves the session variable and
  serializes concurrent requests onto it. The connection string and
  credentials come from `internal/demo/config.go` (overridable via `APP_*`
  env vars).
...
- The `mysql` scanner is attached lazily (`INSTALL mysql; LOAD mysql;`) with
  ...
  `ScannerConnectionString` — lazily (guarded by `sync.Once`) so the app
