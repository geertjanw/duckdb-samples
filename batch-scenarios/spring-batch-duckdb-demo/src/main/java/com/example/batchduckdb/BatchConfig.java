 ...
 *   2. transform — the timed DuckDB vectorized aggregation.
 */
@Configuration
class BatchConfig {
    ...
    private Path summaryCsv() {
        return Path.of(dataDir, "summary-duckdb.csv");
    }
    ...
    Step transformStep(JobRepository jobRepository, PlatformTransactionManager tx) {
        return new StepBuilder("transform", jobRepository)
                .tasklet(new DuckDbTransformTasklet(duckdbUrl, ordersCsv(), summaryCsv()), tx)
                .build();
    }

    @Bean
    Job batchDuckdbJob(JobRepository jobRepository, Step generateStep, Step transformStep) {
