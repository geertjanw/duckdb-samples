# DuckDB ODBC Extension × Axum Demo (Rust)

> Rust/Axum edition. For the same demo built with Spring Boot, see
...
> see [`duckdb-odbc-node-demo`](../duckdb-odbc-node-demo); for Go, see
> [`duckdb-odbc-go-demo`](../duckdb-odbc-go-demo).

Queries MySQL through the **DuckDB ODBC extension**, with DuckDB embedded
in an Axum application:

```
Axum → duckdb crate (in-process) → odbc extension → unixODBC → MariaDB ODBC driver → MySQL
```

Every hop in that chain exists for a reason:

- **Axum → duckdb crate** — the `duckdb` crate embeds the database as a
  bundled C++ library and exposes a rusqlite-style `Connection` API; there is
  no separate JDBC driver in the Rust world, the crate *is* the client.
- **DuckDB (in-process)** — DuckDB is an embedded database: it runs inside
  the Rust process as a library, so there is no DuckDB server to deploy or
  manage.
...
Everything that must be in place before `cargo run` works, and why:

1. **Rust (stable) with a C/C++ toolchain** — the `duckdb` crate is built here
   with its `bundled` feature, which compiles `libduckdb` from vendored C++
   sources via the `cc` crate. That needs a working C/C++ compiler (on macOS the
   Xcode Command Line Tools, `xcode-select --install`, which supply `clang`; on
   Debian/Ubuntu `build-essential`). It does **not** need CMake — that is only
   the separate, experimental `bundled-cmake` feature, which this demo does not
   use. The first build is slow because it compiles the DuckDB amalgamation;
   later builds are cached. DuckDB itself needs no separate install: the crate
   links the native library into your binary, so the only runtime download is
   the `odbc` extension on first use.
...
3. **unixODBC driver manager:**
   ...
   *Additional notes:*
   ...
   - The package also installs two diagnostic tools you'll want:
     ...
     DuckDB and Rust, which cleanly separates "ODBC is broken" from
     "the app is broken".
...
4. **A MySQL-compatible ODBC driver:**
   ...
   *Additional notes:*
   ...
   - The driver's architecture must match the *process* loading it: a
     64-bit Rust binary (hence 64-bit DuckDB) can only load a 64-bit driver `.so`.
   ...
   *Additional notes:*
   ...
   - `odbcinst.ini` (drivers) is not `odbc.ini` (DSNs). This demo uses a
     **DSN-less** connection string — driver name plus `Server`, `Port`,
     ...
     out of `config.rs`, at the cost of another host-specific file.
...
All connection details live in `src/config.rs` and can be overridden with
...
- `odbc_connection_string` is used by the DuckDB `odbc` extension; `{MariaDB}`
...
`src/duckdb_conn.rs` opens a **single shared DuckDB connection** at startup
(`Connection::open_in_memory()`), wraps it in `Arc<Mutex<..>>`, and the ODBC
connection is opened at startup from `odbc_connection_string` plus the
credentials.
...
# 2. Build and start the app. The first build compiles the bundled DuckDB
#    (several minutes); later builds are cached.
cargo run --release
```
...
- **`SET VARIABLE` is per-DuckDB-connection.** The ODBC handle stored via
  ...
  that set it. This is exactly why the demo shares one `Connection` for the
  whole app (`src/duckdb_conn.rs`) instead of opening a connection per request:
  with `:memory:` each new connection is its *own* in-memory database, so
  separate connections wouldn't even share tables or loaded extensions. Because
  `duckdb::Connection` is `Send` but not `Sync`, it lives behind an
  `Arc<Mutex<..>>`; the mutex both makes it shareable and serializes statements
  so the session-variable state can't interleave across concurrent requests.
  (An r2d2 pool would hand out *independent* connections that don't share the
  session variable, so a pool is the wrong tool here.) For production, open the
  ODBC connection per statement instead (pass a connection string directly to
  `odbc_query`/`odbc_copy` with `close_connection = true` — stateless, therefore
  safe to parallelize).

- **Blocking calls go on a blocking thread.** DuckDB calls block, so the axum
  handlers run them through `tokio::task::spawn_blocking` and never hold the
  mutex guard across an `.await`. This keeps the async runtime responsive even
  while a long benchmark query is in flight on the single connection.

- **DECIMAL and HUGEINT are rendered as strings.** The generic row reader in
  `duckdb_conn.rs` converts each cell from its DuckDB runtime type
  (`ValueRef`) into a `serde_json::Value`; `DECIMAL` and `HUGEINT` become
  strings so large/exact values survive JSON without precision loss (hence
  `revenue` comes back as a string like `"125000.50"`), matching the Node and
  Python demos.
...
- **There is no `odbc_exec`.** DDL against the remote DB also goes through
  ...
  `FROM` clause and its result set must be *drained* to force execution:
  `SELECT * FROM odbc_query(getvariable('conn'), 'DROP TABLE IF EXISTS orders')`.
  A bare `SELECT odbc_query(...)` won't parse.
  (An `odbc_exec` exists in the unrelated *nanodbc* community extension —
  easy to confuse when googling.)

- **`odbc_query`'s SQL argument is a constant, not a bind parameter.** The
  demo binds the remote SQL string with a `?` placeholder in
  `odbc_query(getvariable('conn'), ?)`, which DuckDB accepts because it
  folds the parameter to a constant before the table function runs. But you
  cannot parameterize *inside* the remote query that way — the `LIMIT` in
  `top_customers` is built into the SQL string in Rust (it's an `i64`, so it's
  safe), not passed as a bind value to MySQL.
...
- **The scanner ATTACH is one-time per session.** Attaching twice with the
  same alias fails, hence the lazy, guarded (double-checked `AtomicBool`)
...
- **Single DuckDB connection** (`src/duckdb_conn.rs`): the ODBC handle lives
  in a DuckDB session variable (`SET VARIABLE conn = odbc_connect(...)`), which
  is scoped to one connection — see Gotchas. The `Connection` is wrapped in
  `Arc<Mutex<..>>` and shared through axum state; the mutex serializes
  statements so concurrent requests can't interleave on the shared connection.
  The connection string and credentials come from `src/config.rs` (overridable
  via `APP_*` env vars).
